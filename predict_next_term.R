...
PredNextTerm <- function(inStr)
{
   # assign("mesg", "in PredNextTerm", envir = .GlobalEnv)
    ...
    if (inStrLen >= 3)
    {
        ...
        fDF4Temp <- fDF4[grep (searchStr, fDF4$String), ];
    ...
    if (inStrLen >= 2 & nxtTermFound=="FLASE")
    {
        # Assemble the terms of the input string separated by one white space each
        ...
        searchStr <- paste("",inStr1, sep = "");
        fDF3Temp <- fDF3[grep (searchStr, fDF3$String), ];
    ...
    if (inStrLen >= 1 & nxtTermFound==FALSE)
    {
        # Assemble the terms of the input string separated by one white space each
        ...
        searchStr <- paste("",inStr1, sep = "");
        fDF2Temp <- fDF2[grep (searchStr, fDF2$String), ];
  ...
  #    frequently used term from the One Gram using the one gram data frame
    if (inStrLen > 0 & nxtTermFound==FALSE)
    {
        predNxtTerm <- fDF1$String[1];
